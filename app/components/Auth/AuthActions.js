    ...
    axios
      ...
      .then(async (res) => {
        if (res.data.success === true) {
          ...
          dispatch(setUserLogin(true, res?.data?.data?.token))
...
export const setUserLogin = (isLogin, token) => ({
  type: 'SET_USER_DATA',
  ...
  return async (dispatch) => {
    ...
    await axiosInterceptors
      ...
      .then((res) => {
        ...
        dispatch(setUserLogin(false, null))
...
export const getCurrentScreen = (values) => {
  return (dispatch) => {
    dispatch({type: 'GET_CURRENT_SCREEN', payload: values})
  }
}
